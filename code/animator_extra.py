...
def new_animator(MB_start, OB_start, MB_end, OB_end, coast_center, coast_angle):
    ...
    #--- Normal plot (x,y) ---#
    plt.subplot2grid((3,2), (0,0), colspan=1, rowspan=2)
    ...
    #Labels and Legend
    plt.legend(loc='lower center', bbox_to_anchor=(1.1, -0.22), ncol=6)
    plt.xlabel("$x$")
    plt.ylabel("$y$")


    #--- MB plot (x', y') ---#
    plt.subplot2grid((3,2), (0,1), colspan=1, rowspan=2)
    ax01 = plt.gca()
    ...
    ax20 = plt.gca()
    x,y = intercept((MB_pos[i,0], MB_pos[i,1]), (OB_pos[i,0], OB_pos[i,1]), coast_center, coast_angle)
    t = np.linspace(0, len(MB_pos)-1, len(MB_pos))

    theta_data = np.zeros(len(t))
    theta_O_data = np.zeros(len(t))

    for i in range(len(t)):
        theta_data[i] = find_theta(MB_pos, i, x_inter[i], y_inter[i])
    for i in range(len(t)):
        x_inter[i], y_inter[i]
        theta_O_data[i] = find_theta(MB_pos, i, x_inter[0], y_inter[0])

    Delta_theta = np.abs(theta_data - theta_O_data)
    t_neg = np.linspace(0, -len(t)+1, len(t))
    theta_data_rev = theta_data[::-1]

    theta0 = plt.plot([-1000, 1000], [theta_data[0],theta_data[0]], linestyle = "--", color = "black", alpha = 0.5 )
    Delta_theta_display, = plt.plot([0,0], [theta_data[0], theta_O_data[0]], color = color_cycle(4), alpha = 0.8, label = r"$\Delta \theta$ = %.2f" %(Delta_theta[0]))
    theta_O_point, = plt.plot(0, theta_O_data[0], marker = "s", markersize = 8, color = color_cycle(3), label = r"$|\Delta \theta|$ = %.2f" %(Delta_theta[0]))
    theta_O_line, = plt.plot(0, theta_O_data[0], color = color_cycle(3))
    theta_point, = plt.plot(0, theta_data[0], marker = "o", markersize = 5,  color = color_cycle(6), label = r"$\theta_{HB}$ = %.2f" %(theta_data[0]))
    theta_line, = plt.plot(0, theta_data_rev[0], linestyle = "--", color = color_cycle(6))
    my_lines += [theta_O_point, theta_O_line, theta_point, theta_line, Delta_theta_display]


    plt.legend(loc='lower center', bbox_to_anchor=(1.1, -0.2)) #, ncol=6)
    plt.xlabel("$t$ [frames]")
    plt.ylabel(r"$\theta$ [$^{\circ}$]")
    delta_lim = 15
    ymin0, ymax0 = theta_data[0] - delta_lim, theta_data[0] + delta_lim
    ...
    def animate(i):
        ...
        my_lines[12].set_data(0, theta_O_data[i]) #theta_O_data_point
        my_lines[13].set_data(t[:i+1]-i, theta_O_data[:i+1]) #theta_O_data_line
        my_lines[12].set_label(r"$\theta_{O}$ = %.2f" %(theta_O_data[i]))

        my_lines[14].set_data(0, theta_data[i]) #theta_point
        my_lines[15].set_data(t_neg[:i+1], theta_data_rev[-i-1:]) #theta_line
        my_lines[14].set_label(r"$\theta_{HB}$ = %.2f" %(theta_data[i]))

        my_lines[16].set_data([0,0], [theta_data[i], theta_O_data[i]])
        my_lines[16].set_label(r"$\Delta \theta$ = %.2f" %(Delta_theta[i]))

        if i == 0:
            plt.ylim(ymin0, ymax0)
        ymin, ymax = ax20.get_ylim()
        try_ymin = np.min(np.array([theta_data[i], theta_O_data[i]]))
        try_ymax = np.max(np.array([theta_data[i], theta_O_data[i]]))


        #Update labels and legends for ax20
        if try_ymin - 10 < ymin:
            ax20.set_ylim(bottom = try_ymin - 10)
        if try_ymax + 10 > ymax:
            ax20.set_ylim(top = try_ymax + 10)
...
if __name__ == "__main__":
    ...
    new_animator(MB_start, OB_start, MB_end, OB_end, coast_center, coast_angle)
