...
if __name__ == "__main__":
    ...
    manual_limit = [(np.nan, np.nan, np.nan, np.nan), \
                    (np.nan, np.nan, np.nan, np.nan)]  #[Left: (xmin, xmax, ymin, ymax), Right:(xmin, xmax, ymin, ymax)]

    ani = animator(MB_start, OB_start, MB_end, OB_end, coast_center, coast_angle, manual_limit)
    filename = "../article/figures/anic200.gif"
    writer = 'imagemagick'
